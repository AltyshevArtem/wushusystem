...
class Sportsman(models.Model):
    ...
    main_document = models.ImageField(verbose_name="Фото подтверждающего документа")
    city = models.TextField(verbose_name="Название города")
    region = models.ForeignKey(Region, blank=False,  on_delete=models.CASCADE, verbose_name="Название округа/области/края/республики") 
    ...
    rank = models.TextField(Rank, on_delete=models.CASCADE, verbose_name="Наименование ранга")
    rusada = models.ImageField(verbose_name="Изображение антидопингового сертификата")
...
class Sex(models.Model):
    name_sex = models.TextField(primary_key=True, blank=False, verbose_name="Наименование пола")
    class Meta():
...
class Region(models.Model):
    region = models.TextField(blank=False, on_delete=models.CASCADE, verbose_name="Название округа/области/края/республики")
...
class City (models.Model):
    city = models.TextField(primary_key=True, verbose_name="Наименование города")
    region = models.ForeignKey(Region, blank=False, verbose_name="Название округа/области/края/республики")
class Trainer(models.Model):
    ...
    sex = models.ForeignKey(verbose_name="Наименование пола")
...
class Club(models.Model):
    ...
    federation = models.ForeignKey(verbose_name="Федерация, к которой привязан клуб")

class Federation(models.Model):
    name_federation = models.TextField(primary_key=True, verbose_name="Название федерации")
    Region = models.ForeignKey(Region, verbose_name="Название округа/области/края/республики в которой находится федерация")
...
class Duan_Czi_history(models.Model):
    sportsman = models.ForeignKey(Sportsman, on_delete=models.CASCADE, blank=False, verbose_name="Спортсмен")
    new_duan_czi = models.ForeignKey(Duan_Czi, on_delete=models.CASCADE, blank=False, verbose_name="Новый ранг спортсмена Дуань Цзи")
    date = models.DateField(blank=False, verbose_name="Дата выпуска приказа")
    order = models.ImageField(verbose_name="Скан приказа об изменении ранга")
    class Meta():
...
class Club_history(models.Model):
    sportsman = models.ForeignKey(Sportsman, on_delete=models.CASCADE, blank=False, verbose_name="Спортсмены")
    new_club = models.ForeignKey(Club, blank=False, on_delete=models.CASCADE, verbose_name="Новый клуб")
    date = models.DateField(blank=False, verbose_name="Дата смены тренера")
    class Meta():
